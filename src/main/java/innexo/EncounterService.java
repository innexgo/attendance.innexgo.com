...
public class EncounterService {
  ...
  public Encounter getById(int id) {
    String sql =
        "SELECT id, time, location_id, user_id, type FROM encounter WHERE id=?";
    RowMapper<Encounter> rowMapper =
        new BeanPropertyRowMapper<Encounter>(Encounter.class);
  ...
  public List<Encounter> query(Integer count, Integer encounterId,
                               Integer userId, Integer userManagerId, 
                               Integer locationId, Timestamp minTime, 
                               Timestamp maxTime, String userName, String type) {
    String sql =
        ...
        " ORDER BY time DESC" + (count == null ? "" : " LIMIT " + count) + ";";
    RowMapper<Encounter> rowMapper = new EncounterRowMapper();
    return this.jdbcTemplate.query(sql, rowMapper);
  }
  ...
  public void add(Encounter encounter) {
    // Add encounter
    String sql =
        "INSERT INTO encounter (id, time, location_id, user_id, type) values (?, ?, ?, ?, ?)";
    jdbcTemplate.update(sql, encounter.id, encounter.time, encounter.locationId,
                        encounter.userId, encounter.type);

    // Fetch encounter id
    sql =
        "SELECT id FROM encounter WHERE time=? AND location_id=? AND user_id=? AND type=? ORDER BY id DESC";
    List<Integer> id = jdbcTemplate.queryForList(
        sql, Integer.class, encounter.time, encounter.locationId,
        encounter.userId, encounter.type);
  ...
  public void update(Encounter encounter) {
    String sql =
        "UPDATE encounter SET id=?, time=?, location_id=?, user_id=?, type=? WHERE id=?";
    jdbcTemplate.update(sql, encounter.id, encounter.time, encounter.locationId,
