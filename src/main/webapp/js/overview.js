...
async function manualEncounter(studentId) {
  ...
  let locationId = getLocalJson('default-locationid');
  if (course != null) {
...
async function locationOptions() {
  ...
  while (true) {
    try {
      ...
      let defaultLocation = getLocalJson('default-locationid');
      ...
      locationSelect.change(async function() {
        ...
        if (selectedValue != null) {
          setLocalJson('default-locationid', selectedValue);
        } else {
...
async function currentStatus() {
  ...
  while (true) {
    let period = getLocalJson('period');
    let courses = getLocalJson('courses').sort((a, b) => (a.period > b.period ? -1 : 1));
    let course = period == null ? null : courses.filter(c => c.period == period.numbering)[0];
